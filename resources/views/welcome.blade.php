...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="antialiased">
     <div class="demo">
        <div class="container">
            <div class="row">
                <div class="col-md-offset-1 col-md-10" style="flex: 0 0 100%; max-width:100%;">
                    <div class="panel">
                        <div class="panel-heading">
                            <div class="row">
                                <div class="col col-sm-6 col-xs-12">
                                    <h4 class="title">Countries <span>Catalog</span></h4>
                                </div>
                                <div class="col-sm-6 col-xs-12 text-right">
                                    <div class="btn_group">
                                        <form method="get" >
                                            <div class="input-group vr-head-form-sort ">
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    @foreach ($paginatedData as $index=>&$item )
                                        <tr>
                                            <td>{{ $index+1 }}</td>
                                            <td class="table-col-img"><img src="{{ $item->flags->png }}" alt=""></td>
                                            <td class="btn_popup">{{ $item->name->official }}</td>
                                            <td>{{ $item->cca2 }}</td>
                                            <td>{{ $item->cca3 }}</td>
                                            <td>
                                                @if (isset($item->name->nativeName))
                                                    @foreach ($item->name->nativeName as$i )
                                                        {{ $i->official }}
                                                        {{ $i->common }}
                                                    @endforeach
                                                @endif
                                            ...
                                            <td>
                                                @foreach ($item->altSpellings as$i )
                                                    {{ $i }}
                                                @endforeach
                                            </td>
                                            <td>
                                                <ul class="action-list">
                                                    @if (isset($item->idd->root))
                                                        <li>Root: {{ $item->idd->root }}</li>
                                                    @endif
                                                    @if (isset($item->idd->suffixes[0]))
                                                        <li>Suffixes: {{ $item->idd->suffixes[0] }}</li>
                                                    @endif
                                                </ul>
                                            </td>
                                        </tr>
                        ...
                        <div class="panel-footer">
                            <div class="row">
                                ...
                                @endphp
                                <div class="col col-sm-6 col-xs-6">showing <b>{{ $count }}</b> out of <b>{{ $paginatedData->total() }}</b> entries</div>
...
<script src="https://kit.fontawesome.com/77796b0c5f.js" crossorigin="anonymous"></script>
